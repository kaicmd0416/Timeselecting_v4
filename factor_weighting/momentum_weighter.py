...
class MomentumWeighter(BaseWeighter):
    """
    动量加权器
    ...
    def __init__(self, lookback_window: int = 252, min_periods: int = 252, decay: float = 0.0):
        """
        初始化动量加权器
    ...
    def calculate_weights(self, df_signals: pd.DataFrame, df_returns: pd.DataFrame,
        ...
        for factor in factor_names:
            ...
            if self.decay > 0:
                ...
                cum_return = (factor_return * weights).sum()
            else:
                # 简单累计收益
                cum_return = (1 + factor_return).prod() - 1

            momentum_values[factor] = cum_return

        momentum_series = pd.Series(momentum_values)

        # 累计收益为负的设为0
        momentum_series[momentum_series < 0] = 0

        # 归一化权重
        weights = self.normalize_weights(momentum_series, remove_negative=True)
